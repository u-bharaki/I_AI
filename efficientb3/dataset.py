...
def balanced_dataset(df):
    ...
    for cls in range(NUM_CLASSES):
        ...
        ds = ds.map(decode_and_resize, num_parallel_calls=tf.data.AUTOTUNE) #resmi boyutlandır
        ds = ds.map(augment, num_parallel_calls=tf.data.AUTOTUNE) #augmentation
        datasets.append(ds)

    final = tf.data.Dataset.sample_from_datasets(datasets, weights=None)
    final = final.shuffle(4000).batch(BATCH_SIZE).prefetch(tf.data.AUTOTUNE)
    return final
